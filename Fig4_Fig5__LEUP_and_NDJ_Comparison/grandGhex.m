function [g]=grandGhex(LEUPval)
NN=size(LEUPval,1);
LEUPn=LEUPval/max(LEUPval(:));
...
for rr=1:floor(NN/2)
[NNx1,NNy1,NNn1]= MooreNeighbours(rr);
[NNx2,NNy2,NNn2]= MooreNeighbours(rr-1);
...
for ll=1:NN
    for mm=1:NN
        ...
        for ii=1:NNn
             nxval=mod((-1)^(mod(ll+1,2))*NNy(ii)+ll,NN);
            if nxval==0
                nxval=NN;
            end
            
            nyval=mod((-1)^(mod(ll+1,2))*NNx(ii)+mm,NN);
            if nyval==0
                nyval=NN;
            end
...
ncluster=ncluster+1;
        ...
        gn(ll,mm,rr)=(ncluster/NNn)/(phen/576);
    end
end
end
